...
getData() {
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        if [[ -f ~/v2ray.pem && -f ~/v2ray.key ]]; then
            ...
            # resolve = $(curl -sH 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=$new_domain&type=A" | grep -oE "([0-9]{1,3}\.){3}[0-9]{1,3}" | head -1)
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        echo "   2) 小说站(随机选择(空))"
        echo "   3) 马克思站(https://www.marxists.org/chinese/maozedong/)"
        echo "   4) 高清壁纸站(https://unsplash.com/)"
        echo "   5) 自定义反代站点(需以http或者https开头)"
        read -p "  请选择伪装网站类型[默认:高清壁纸站]" answer
        if [[ -z "$answer" ]]; then
            # PROXY_URL="https://www.bing.com/images"
            ...
            2)
                ...
                ;;
            3)
                PROXY_URL="https://www.marxists.org/chinese/maozedong/"
                ;;
            4)
                PROXY_URL="https://unsplash.com/"
                ;;
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        curl -sL https://get.acme.sh | sh -s email=zhouqinya@outlook.com
        source ~/.bashrc
